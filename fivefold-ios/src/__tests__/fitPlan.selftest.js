...
const { toModel, fixableOverlaps, allowedIds, cascadePlan, validatePlan, buildMessages, parsePlanText, describePlan, staysFor, hm } = mod;
let failures = 0;
...
check(msgs[0].role === 'system' && /ONE JSON object/.test(msgs[0].content) && /5-minute steps/.test(msgs[0].content), 'system prompt demands JSON on the grid');
check(/Errand \| 16:00-16:45 \| JUST ADDED, must stay/.test(u) && /Newcastle United vs Liverpool \| 16:30-18:30 \| FIXED \(kick-off is fixed\)/.test(u) && /Pull \| 16:00-17:00 \| movable/.test(u) && /take shower \| .* \| not involved, leave it/.test(u) && /4th Prayer \| .* \| short, ignore/.test(u) && /eat dinner \| .* \| FIXED \(repeats every week\)/.test(u), 'every item is labelled honestly for the model');
