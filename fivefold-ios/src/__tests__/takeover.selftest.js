...
check(T.applyTemplateDay(ecWed, T.parseKeepNotes(T.keepNotes(['Work', 'Social Media time'])).titles).map((i) => i.title).join() === '1st Prayer,Work,Social Media time,Show', 'EyeCandy: the day marker\'s keep list hides the breakfast mirror, keeps Work and Social Media');
check(T.keepNotes(['Work', 'Social Media time']) === 'Added by Biblely · template · keep:work|social media · hide:' && T.parseKeepNotes('Added by Biblely · reminder') === null && JSON.stringify(T.parseKeepNotes(T.keepNotes(['Work'], ['prayers', 'sports']))) === '{"titles":["work"],"hide":["prayers","sports"]}' && JSON.stringify(T.parseKeepNotes('Added by Biblely · template · keep:work|social media')) === '{"titles":["work","social media"],"hide":[]}', 'day marker notes round-trip (old markers without hide still parse)');
...
check(/applyTemplateDay\(applyTakeover\(dedupeMirrors\(out\)\), keep, hide\)/.test(src('utils/dayItems.js')) && /if \(td\) \{ keep = td\.keep; hide = td\.hide; \}/.test(src('utils/dayItems.js')), 'Biblely loader applies it from the day\'s template');
check(/templateDayTitles\(takeoverTitles\(out, blockTitles\), keep, hide\)/.test(src('utils/dayBusy.js')) && /'reminder', r\.type !== 'one-time'\)/.test(src('utils/dayBusy.js')), 'busy lists too');
...
check(/parseKeepNotes\(e\.notes\)/.test(ecdi) && /applyTemplateDay\(applyTakeover\(dedupeMirrors\(out\)\), keep, hide\)/.test(ecdi), 'EyeCandy reads the marker and applies it');

// Groups a template turns off
check(T.groupOf({ kind: 'prayer', raw: {} }) === 'prayers' && T.groupOf({ kind: 'gym', raw: {} }) === 'workouts' && T.groupOf({ kind: 'task', raw: {} }) === 'oneOffs' && T.groupOf({ kind: 'reminder', raw: { type: 'one-time' } }) === 'oneOffs' && T.groupOf({ kind: 'reminder', raw: { type: 'recurring' } }) === null && T.groupOf({ kind: 'calendar', raw: { recurring: false } }) === 'oneOffs' && T.groupOf({ kind: 'eyecandy', raw: {} }) === 'eyecandy' && T.groupOf({ kind: 'eyecandySports', raw: {} }) === 'sports' && T.groupOf({ kind: 'biblely', raw: { biblelyKind: 'prayer' } }) === 'prayers' && T.groupOf({ kind: 'biblely', raw: { biblelyKind: 'todo' } }) === 'oneOffs' && T.groupOf({ kind: 'block', raw: {} }) === null, 'every kind maps to its group');
const off = T.applyTemplateDay(wed, ['Work', 'Social Media time'], ['prayers', 'sports', 'oneOffs']).map((i) => i.title);
check(off.join() === 'Work,Social Media time,Push day', `prayers, matches and one-offs turned off: only the template and the workout stay (${off.join()})`);
check(T.templateDayTitles([{ title: 'Prayer', source: 'prayer' }, { title: 'Push', source: 'gym' }, { title: 'Dentist', source: 'calendar', recurring: false }, { title: 'Work', source: 'calendar', recurring: true }], ['Work'], ['prayers', 'oneOffs']).map((e) => e.title).join() === 'Push,Work', 'busy lists drop the turned-off groups');
check(/hideGroupsFor\(t\)\)/.test(src('services/calendarSync.js')) && /parseKeepNotes\(e\.notes\); if \(k\) \{ keep = k\.titles; hide = k\.hide; \}/.test(ecdi) && /applyTemplateDay\(applyTakeover\(dedupeMirrors\(out\)\), keep, hide\)/.test(ecdi), 'marker carries the hide list; EyeCandy applies it');
const ecn = fs.readFileSync(path.join(root, '..', '..', '..', 'eyecandy', 'src', 'services', 'notificationService.js'), 'utf8');
check(/export const biblelyQuietDays/.test(ecn) && /if \(quietOn\(quiet, key, mediaType === 'sport'\)\)/.test(ecn) && /quietOn\(quiet, keyOfDate\(nextOcc\)/.test(ecn) && /export const refreshIfQuietDaysChanged/.test(ecn) && /refreshIfQuietDaysChanged\(\)/.test(fs.readFileSync(path.join(root, '..', '..', '..', 'eyecandy', 'App.js'), 'utf8')), 'EyeCandy: shows/matches stay quiet on days Biblely turned them off (one-offs skipped, weekly pushed a week); refresh on foreground');
const nsrc = src('services/notificationService.js');
check(/hiddenDays\('prayers', 15\)/.test(nsrc) && /hiddenPrayers\.has\(dateKeyOfLocal\(occurrence\)\)/.test(nsrc) && /hiddenPrayers\.has\(dateKeyOfLocal\(occ\)\)/.test(nsrc) && /hiddenDays\('oneOffs', 21\)/.test(nsrc) && /hiddenTasks\.has\(dateKeyOfLocal\(taskDate\)\)/.test(nsrc) && /hiddenOnce\.has\(dateKeyOfLocal\(triggerDate\)\)/.test(nsrc), 'Biblely: prayers, tasks and one-time reminders stay quiet on days their group is off');
check(/hiddenDatesForGroup\('workouts', 28\)/.test(src('services/workoutSchedule.js')) && /skipDates\.includes\(nextKey\) \|\| hidden\.has\(nextKey\)/.test(src('services/workoutSchedule.js')) && /scheduleWorkoutNotifications\(schedule\); count\+\+/.test(nsrc), 'workouts quiet on days turned off; the sweep uses the one scheduler');
check(/DAY_GROUPS\.map\(\(g\) =>/.test(src('screens/DayTemplatesScreen.js')) && /Also on this kind of day/.test(src('screens/DayTemplatesScreen.js')), 'editor: On/Off per group');
