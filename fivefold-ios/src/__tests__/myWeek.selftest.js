...
check(/const stamp = \(id\) => \(\{ id, recurring: d\.recurring, start: d\.start\.getTime\(\), end: d\.end\.getTime\(\), title: d\.title, skips, alarmsOff: calendarAlertsOff, notes: details\.notes, cal: wantCal \}\);/.test(cs) && /const same = !FORCE && entry/.test(cs) && /\(entry\.notes \|\| 'Added by Biblely'\) === details\.notes/.test(cs) && /entryCal !== wantCal/.test(cs), 'mirror entries remember what was written (notes + calendar too); unchanged events are left alone, calendar changes recreate');
