...
check(/static let hours = 5/.test(sw) && /enum MyWeekTimeline/.test(sw) && /struct MyWeekBlock/.test(sw) && /minimumScaleFactor\(0\.55\)/.test(sw) && /let strip = p\.strip/.test(sw) && /let stripMax = 24/.test(sw) && /TOMORROW/.test(sw), 'large widget: real 5-hour timeline, side-by-side overlaps, text shrinks instead of cutting, strips for tiny items, runs past midnight');
