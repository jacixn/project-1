...
check(/static let hours = 7/.test(sw) && /enum MyWeekTimeline/.test(sw) && /struct MyWeekBlock/.test(sw) && /minimumScaleFactor\(0\.6\)/.test(sw) && /let strip = height < 17/.test(sw) && /TOMORROW/.test(sw), 'large widget: real 7-hour timeline, side-by-side overlaps, text shrinks instead of cutting, strips for tiny items, runs past midnight');
