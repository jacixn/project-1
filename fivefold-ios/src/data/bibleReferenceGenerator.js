 ...
 * 
 * Generates all 31,102 Bible verse references from a compact data structure.
 * This keeps the app lightweight while providing access to every verse.
 */

// Compact Bible structure: [book name, chapters, verses per chapter]
const BIBLE_BOOKS = [
  // Old Testament
...
/**
 * Bible Reference Generator Class
 */
class BibleReferenceGenerator {
  /**
   * Generate all Bible verse references
   * @returns {Array} Array of all 31,102 verse references
   */
  static getAllVerseReferences() {
    const allReferences = [];
    
    for (const book of BIBLE_BOOKS) {
      for (let chapter = 1; chapter <= book.chapters; chapter++) {
        const versesInChapter = book.verses[chapter - 1];
        
        for (let verse = 1; verse <= versesInChapter; verse++) {
          allReferences.push(`${book.name} ${chapter}:${verse}`);
        }
      }
    }
  ...
  /**
   * Get references for a specific book
   * @param {string} bookName - Name of the book
   * @returns {Array} Array of verse references for the book
   */
  static getBookReferences(bookName) {
    ...
    for (let chapter = 1; chapter <= book.chapters; chapter++) {
      ...
      for (let verse = 1; verse <= versesInChapter; verse++) {
        references.push(`${book.name} ${chapter}:${verse}`);
      }
    }
  ...
  /**
   * Get random verse references
   * @param {number} count - Number of references to return
   * @returns {Array} Array of random verse references
   */
  static getRandomReferences(count = 1) {
    const allReferences = this.getAllVerseReferences();
   ...
   * @param {string} reference - Reference to validate (e.g., "John 3:16")
   * @returns {boolean} True if reference exists
   */
  static isValidReference(reference) {
    const allReferences = this.getAllVerseReferences();
    return allReferences.includes(reference);
  }
  
  /**
   * Get Bible statistics
   * @returns {Object} Statistics about the Bible
   */
  static getBibleStats() {
    ...
    for (const book of BIBLE_BOOKS) {
      ...
      totalVerses += book.verses.reduce((sum, verses) => sum + verses, 0);
    }
  ...
  /**
   * Get all book names
   * @returns {Array} Array of all book names
   */
  static getAllBookNames() {
    return BIBLE_BOOKS.map(book => book.name);
  }
}

export default BibleReferenceGenerator;