...
const ProfileTab = () => {
  ...
  const [calAlarmMin, setCalAlarmMin] = useState(10); // default calendar-app alert lead time
  ...
  // 2FA row action — lifted out of the Settings hub so the new native-stack
  // SettingsScreen can trigger it via the 'openSettingsChild' event (the hub no
  // longer holds this branching logic).
  const handleTwoFactorPress = () => {
    if (!emailVerified) {
      Alert.alert(
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          { text: 'Verify Now', onPress: () => navigation.navigate('EmailVerification', { fromSignup: false, maskedEmail: user?.email || '' }) },
        ]
      );
      return;
    }
  ...
  // SettingsScreen (native-stack modal hub) dismisses itself then emits this so
  // each child sheet presents from ProfileTab's view controller — the correct
  // top VC once the hub sheet is gone, and never two RN sheets at once.
  useEffect(() => {
    const sub = DeviceEventEmitter.addListener('openSettingsChild', (payload) => {
      switch (payload?.key) {
        case 'referral': setShowReferralModal(true); break;
        case 'bibleVersion': setShowBibleVersionModal(true); break;
        case 'language': setShowLanguageModal(true); break;
        case 'voice': setShowVoicePickerModal(true); break;
        case 'notifications': setShowNotificationSettings(true); break;
        case 'twoFactor': handleTwoFactorPress(); break;
        case 'emailVerify': navigation.navigate('EmailVerification', { fromSignup: false, maskedEmail: user?.email || '' }); break;
        case 'adminAnalytics': setShowAdminAnalytics(true); fetchAttributionData(); break;
        case 'adminReports': setShowAdminReports(true); fetchReportsData(); break;
        case 'adminFeedback': setShowAdminFeedback(true); fetchFeedbackEntries(); break;
        default: break;
      }
    });
    return () => sub.remove();
  }, [emailVerified, twoFactorEnabled, user, navigation]);
  ...
  const LegalSection = () => (
    <AnimatedSettingsCard style={styles.aboutCard}>
      <View style={{ padding: 16 }}>
        ...
        <TouchableOpacity
          onPress={() => { hapticFeedback.buttonPress(); navigation.navigate('Legal', { kind: 'privacy' }); }}
        ...
        <TouchableOpacity
          onPress={() => { hapticFeedback.buttonPress(); navigation.navigate('Legal', { kind: 'terms' }); }}
          style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 14, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)' }}
          activeOpacity={0.6}
        >
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        <TouchableOpacity
          onPress={() => { hapticFeedback.buttonPress(); navigation.navigate('Legal', { kind: 'support' }); }}
          style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 14, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)' }}
          activeOpacity={0.6}
        >
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        <TouchableOpacity
          onPress={() => { hapticFeedback.buttonPress(); navigation.navigate('Legal', { kind: 'bibleCredits' }); }}
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        onDismiss={() => setShowBibleVersionModal(false)}
      >
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                hapticFeedback.buttonPress();
                setShowSettingsModal(false);
                setTimeout(() => navigation.navigate('CustomiseCards'), 300);
              }}
                activeOpacity={0.7}
            >
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
      ...
      <Modal
        ...
        animationType="slide"
        presentationStyle="pageSheet"
        onRequestClose={() => { setShow2FASetupModal(false); setTwoFASetupCode(''); }}
        onDismiss={() => { setShow2FASetupModal(false); setTwoFASetupCode(''); }}
      >
        <KeyboardAvoidingView 
      ...
      <Modal
        ...
        animationType="slide"
        presentationStyle="pageSheet"
        onRequestClose={() => { setShowReferralModal(false); setReferralUsername(''); }}
        onDismiss={() => { setShowReferralModal(false); setReferralUsername(''); }}
      >
        <KeyboardAvoidingView 
          style={{ flex: 1, backgroundColor: theme.background }}
          behavior={Platform.OS === 'ios' ? 'padding' : 'height'}
